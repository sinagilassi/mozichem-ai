...
### Tips for MCP Configuration

- **Test connectivity**: Ensure remote servers are accessible at their URLs
- **Environment variables**: Use the `env` field for API keys and configuration
- **Enable/disable**: Use the `enabled` field to quickly turn servers on/off
- **Descriptive names**: Use clear, descriptive names for easier management
- **Local paths**: For stdio servers, ensure commands and paths are correct for your system
